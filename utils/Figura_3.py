...
def test(opt):
    ...
    model_conv.load_weights(str(str(Path(__file__).parent.parent) + '/weights/200-epoch-chirp-multi-channel/best.ckpt')).expect_partial()
    ...
    j = input("index 2 data show: ")
    while True:
        if i != "" and j!= "":
            # Agregar contador de tiempo para DAE y FISTA
            image_index = [int(i), int(j)]
            if(opt.authors):
                x_hat, y_hat = model_conv.call(x[image_index[0]][None,:,:,:])
                x_hat1, y_hat1 = model_conv.call(x[image_index[1]][None,:,:,:])
            else:
                x_hat, y_hat = model_conv.call(x[image_index[0]][None,:,:])
                x_hat1, y_hat1 = model_conv.call(x[image_index[1]][None,:,:])
