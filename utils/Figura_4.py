...
################################################################################
# Global consts
default_kernel = 'kernels/kernel.npy' # For simulated chirped data
default_data = 'data/CHIRP_DAS_FASE_data.h5'
...
def test(opt):
    ...
    while True:
        if i != "":
            ...
            image_index = int(i)
            if(opt.authors):
                x_hat, y_hat = model_noflip.call(x[image_index][None,:,:,:])
                x_hat1, y_hat1 = model_flip.call(x[image_index][None,:,:,:])
            else:
                x_hat, y_hat = model_noflip.call(x[image_index][None,:,:])
                x_hat1, y_hat1 = model_flip.call(x[image_index][None,:,:])
            ...
            # Draw canvas
            plt.close("all")
            fig, axes = plt.subplots(ncols=2, nrows=3, figsize=(9, 4), constrained_layout=True, sharex="col", sharey="row")
            ...
            for i, wv in enumerate(x[image_index]):
                ax.plot(t, 1*wv - scale * i, c="k")
            ax.set_title('No-Flip: Cross-Correlation', fontsize=14)
            ax = axes[1, 0]
            for i, wv in enumerate(x_hat):
                ax.plot(t, 16*wv - scale *i, c="k")
            ax = axes[2, 0]
            for i, wv in enumerate(y_hat):
                ax.plot(t, 1*wv - scale * i, c="k")
            ax = axes[0, 1]
            ax.set_title('Flip: Convolution', fontsize=14)
            for i, wv in enumerate(x[image_index]):
                ax.plot(t, 1*wv - scale * i, c="k")
            ax = axes[1, 1]
            ...
            # 1379.52 362.16
            plt.tight_layout()
            fig.savefig('Figuras/Figura_4.pdf')
            plt.close()
        else:
            break
        i = input("index 1 data show: ")
